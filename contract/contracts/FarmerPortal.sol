...
contract FarmerPortal {
    enum UserRole { Unregistered, Farmer, Buyer, Broker }
    ...
    modifier onlyBroker() {
        require(users[msg.sender].registered, "User not registered");
        require(users[msg.sender].role == UserRole.Broker, "Only brokers allowed");
        _;
    }

    function registerUser(uint8 role) external {
        require(role == uint8(UserRole.Farmer) || role == uint8(UserRole.Buyer) || role == uint8(UserRole.Broker), "Invalid role");
